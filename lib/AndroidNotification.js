...
class AndroidNotification extends UmengNotification {
    ...
    public static isPayloadKey(key) {
        return utils.containsValue(PAYLOAD_KEYS, key);
    }

    public static isBodyKey(key) {
        return utils.containsValue(BODY_KEYS, key);
    }

    public static AfterOpenAction = AfterOpenAction;
    ...
    public setExtraField(key, value) {
        let payloadJson = null;
        let extraJson = null;
        if (this.rootJson.hasOwnProperty("payload")) {
    ...
    //
    public setDisplayType(type) {
        this.setPredefinedKeyValue("display_type", type + '');
    }

    ///通知栏提示文字
    public setTicker(ticker) {
    ...
    public setBuilderId(builder_id) {
    ...
    public setAfterOpenAction(action) {
